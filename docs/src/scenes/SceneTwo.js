...
export default class SceneTwo extends Phaser.Scene{
    ...
    loadAssets(){
        ...
        this.load.image('PipeTriple', 'assets/PipeTripleExplanation.png')
        ...
        this.load.spritesheet('Boy_idle', 'assets/spritesheet/Boy_idle.png', {
            frameWidth,
            frameHeight
        });
    ...
    loadAnimation(){
        ...
        this.anims.create({
            key: 'Boy_idle',
            frames: this.anims.generateFrameNumbers('Boy_idle', {start:0, end:1}),
            frameRate: 4,
            repeat: -1
        })
    }
    ...
    tutorialKeys(){
        const PipeTriple = this.add.image(this.width/2, this.height * 0.2,'PipeTriple').setDepth(99).setScale(3*Constants.scaleFactor)
    ...
    grannyAnimation(x,y){
        this.grannySprite = this.add.sprite(x,y, 'Boy_idle')
        .setScale(10*this.scaleFactor)
        .setDepth(10)
        .setFlipX(true)
    }
    ...
    createDictionary(){
        this.Dictionary = [
            ...
            {
                text: "Can you take a look at this pipe system?",
                speaker: 'client'
            },
            {
                text: "Wow, such a mess.\nHow's an idiot, who made it?",
                speaker: 'moti'
            },
            {
                text: "You",
            ...
            {
                text: "Anyway, can you connect two sources to one sink?",
                speaker: 'client'
            }
            ,
            {
                text: "Let's see what I can do.",
                speaker: 'moti'
            }
        ]
    }
    ...
    animateText(){
        ...
        if (speaker === 'client'){
            this.grannySprite.play('Boy_idle')
            this.installatorSprite.anims.stop(true)
            this.game.events.emit('playSoundSpeak', 'boy')
        }
    ...
    pauseButton(){
        ...
        pause_button.on('pointerdown', () => {
            this.game.events.emit('buttonClicks')
            this.scene.launch('Pause', { param: 'SceneTwo' })

            if (!this.multiPlayerState) {this.scene.pause('SceneTwo')}
        })
    }
