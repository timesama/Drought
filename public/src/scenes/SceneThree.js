...
export default class SceneThree extends Phaser.Scene{
    ...
    loadAssets(){
        ...
        this.load.spritesheet('Enemy_idle', 'assets/spritesheet/Enemy_idle.png', {
            frameWidth,
            frameHeight
        });
    ...
    tutorialKeys(){
        const keySpace = this.add.image(this.width/2, this.height * 0.2,'KeyThrow').setDepth(99).setScale(5*Constants.scaleFactor)
    ...
    grannyAnimation(x,y){
        this.grannySprite = this.add.sprite(x,y, 'Enemy_idle')
        .setScale(10*this.scaleFactor)
        .setDepth(10)
        .setFlipX(true)
    }
    ...
    createDictionary(){
        this.Dictionary = [
            ...
            {
                text: "I will destroy everything you do!",
                speaker: 'client'
            },
            {
    ...
    animateText(){
        ...
        addNextLetter()
        createNextButton()
        this.grannySprite.play('Enemy_idle')
        
        if (speaker === 'client'){
            this.installatorSprite.anims.stop(true)
            this.game.events.emit('playSoundSpeak', 'enemy')
        }
    ...
    pauseButton(){
        ...
        pause_button.on('pointerdown', () => {
            this.game.events.emit('buttonClicks')
            this.scene.launch('Pause', { param: 'SceneThree' })

            if (!this.multiPlayerState) {this.scene.pause('SceneThree')}
        })
    }
